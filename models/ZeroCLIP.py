"""
DISCLAIMER:
The authors don't own any rights for the code or data. 
Code taken from: https://github.com/YoadTew/zero-shot-image-to-text
"""
...
# !IMPORTANT!
# get_args should be changed to reflect the location of
# "forbidden_tokens.npy" file. 
    
def get_args(gen_type):
    args = {}
    args['lm_model']="gpt-2"
    args['forbidden_tokens_file_path']='models/forbidden_tokens.npy'
...
class CLIPTextGenerator:
    def __init__(self,
                 seed=0,
                 lm_model='gpt-2',
                 forbidden_tokens_file_path='models/forbidden_tokens.npy',
    ...
    """
    Function that takes in the image itself as an input instead of the image path.
    Useful to link with our custom dataset. The contents of this function is simply
    a copy of the function "get_image_feature" without the process of opening
    the image using PIL from the img_path. 
    """
    def get_img_feature_no_path(self, imgs, weights):
    ...
    def generate_text(self, context_tokens, beam_size):
        ...
        for i in range(self.target_seq_length):
            ...
            log_info(tmp_output_texts, verbose=self.verbose)
            ####
